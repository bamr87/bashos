# Terminal Enchantment: Oh-My-Zsh Setup Guide
...
```bash
note Fixed the login bug    # Saves timestamped note to ~/.dev-notes/
```
...
| `~/Library/Fonts/` | MesloLG Nerd Font files |
| `~/Library/Application Support/Code/User/settings.json` | VSCode terminal config |

## Validation

Run the health check:
```bash
~/setup-terminal.sh --dry-run   # Shows health check at the end
~/validate-plugins.sh           # Validates all 9 plugins
~/validate-vscode-terminal.sh   # Validates VSCode integration
```

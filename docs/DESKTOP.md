...
## The GUI desktop (`bashos gui`)

The Textual surface above is the TTY default. `bashos gui` is a second access layer on the **same kernel**: a native window (or your browser) that renders scenes over loopback HTTP + SSE. It never reasons, never widens policy, and never executes a shell line.

...
||||||| Stash base
The desktop is bashOS's interactive surface: a windowed terminal environment built on [Textual](https://textual.textualize.io), replacing the line REPL. It is the L6 interface layer from the reference architecture — the promised "TUI dashboard" matured into the primary surface. It is a *client* of the kernel and the engine: it builds payloads, renders results and typed engine
||||||| Stash base
The desktop is bashOS's interactive surface: a windowed terminal environment built on [Textual](https://textual.textualize.io), replacing the line REPL. It is the L6 interface layer from the reference architecture — the promised "TUI dashboard" matured into the primary surface. It is a *client* of the
||||||| Stash base
The desktop is bashOS's interactive surface: a windowed terminal environment built on [Textual](https://textual.textualize.io), replacing the line REPL. It is the L6 interface layer from the reference architecture — the promised
...
||||||| Stash base
Everything runs offline: `KernelConfig(dry_run=True)` gives llm=None (no auth, no network, no engine — loops short-circuit to their rendered-prompt reports), and FakeChat pins exact model-call counts. Pilot drives the UI headless; three SVG snapshots (`pytest --snapshot-update` after intentional visual changes) are the visual regression net.
<<<<<<< Updated upstream
======= Everything runs offline: `KernelConfig(dry_run=True)` gives llm=None (no auth, no network, no engine — loops short-circuit to their rendered-prompt reports), and FakeChat pins exact model-call counts. Pilot drives the UI headless; three SVG snapshots (`pytest --snapshot-update` after intentional visual changes) are the visual regression net.
||||||| e147539
======= A GUI front end on the same kernel the terminal runs. Read [HARNESS.md](HARNESS.md) for the core architecture — this document covers the window: what it is, what it deliberately is not, and how it is wired. For the feature-by-feature walkthrough with screenshots and recordings, see [DESKTOP-TOUR.md](DESKTOP-TOUR.md).
...
>>>>>>> Stashed changes
||||||| Stash base
Everything runs offline: `KernelConfig(dry_run=True)` gives llm=None (no auth, no network, no engine — loops short-circuit to their rendered-prompt reports), and FakeChat pins exact model-call counts. Pilot drives the UI headless; three SVG snapshots (`pytest --snapshot-update` after intentional
